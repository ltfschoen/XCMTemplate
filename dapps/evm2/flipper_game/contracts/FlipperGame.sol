...
contract FlipperGame {
    ...
    uint256 private constant BLOCKS_ALLOW_RANDOMNESS = 12; // 70 sec @ 6 sec/block
    uint256 private constant BLOCKS_ALLOW_FULFILL = 7; // 40 sec @ 6 sec/block
    ...
    mapping(uint256 => PlayerGuessStruct[]) public playerGuessesForPlayerOfGameId;
    ...
    event CreatedGame(uint256 indexed gameId, bytes32 blockHashPrevious, address indexed createdBy,
        uint256 indexed createdAtBlock, uint256 endGuessesAtBlock);
    event AddedPlayerToGame(uint256 indexed gameId, address indexed playerAddress, uint256 indexed createdAtBlock);
    event AddedGuessForPlayerOfGame(uint256 indexed gameId, uint256 indexed playerGuessId,
        address indexed guessByPlayerAddress, uint256 guess, uint256 createdAtBlock);
    event AddedAnswerForGame(uint256 indexed gameId, uint256 indexed answerId, address answerByAddress,
        uint256 answer, uint256 indexed createdAtBlock);
    ...
    function createGame(uint256 _initialGuess) external payable {
        require(_initialGuess <= 20, "initial guess too high");
        ...
        gameForGameId[gameId] = gameInstance;
        gamesList.push(gameInstance);    

        emit CreatedGame(gameId, blockHashPrevious, msg.sender, blockNumber, blockNumber + BLOCKS_ALLOW_GUESS);
    }
    ...
    function hasPlayerForGameId(uint256 _gameId) public view returns (bool) {
        for (uint256 i = 0; i < playersForGameId[_gameId].length; i++) {
    ...
    function hasPlayerGuessedForGameId(uint256 _gameId) public view returns (bool) {
        for (uint256 i = 0; i < gameIdsForPlayerId[msg.sender].length; i++) {
    ...
    function addGuessForPlayerOfGame(uint256 _gameId, uint256 _guess) internal returns(uint256) {
        ...
        emit AddedGuessForPlayerOfGame(_gameId, playerGuessId, msg.sender, _guess, blockNumber);
